...
def Caller():
  ...
  x_org = list(np.zeros([1000,1]))
  y_org = list(np.linspace(20,-20,1000))
  ...
  for i in range(len(y_org)):
    ...
    vertex.pose.position.x= float(x_org[i])
    vertex.pose.position.y= float(y_org[i])
    vertex.pose.position.z= 0.0

    vertex.pose.orientation.x= 0.0 #angle
    vertex.pose.orientation.y= 0.0 #first derivative
    vertex.pose.orientation.z= 0.0 #second derivative
