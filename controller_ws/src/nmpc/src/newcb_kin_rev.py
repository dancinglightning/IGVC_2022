...
class NMPController():
    def __init__(self, N_ref = 1):
        ...
        self.La_rev = self.La       # distance of front tires from COM in m
        self.Lb_rev = self.Lb       # distance of back tires from COM in m
        ...
        self.cost = 0               # cost at every iteration
        self.gear = 0               # gear mode
        self.cost_reverse = 100     # cost lower limit to initiate reverse


        #MPC  variables
        self.model = self.forward_model()
        self.model_rev = self.reverse_model()
    ...
    def forward_model(self):
        ...
        equations = vertcat( v * np.sin(psi), 
                           v * np.cos(psi),
                           a ,
                           (v/self.Lb) * np.tan(delta),
                           w
                         )
        ...
        model.set_rhs('psi',equations[3])
        # model.set_rhs('psi_dot',equations[4])
        model.set_rhs('delta',equations[4])
    ...
    def reverse_model(self):
        ...
        #control inputs
        a_rev = model.set_variable(var_type='_u',var_name='a_rev',shape = (1,1))        # reverse acceleration
        w_rev = model.set_variable(var_type='_u',var_name='w',shape = (1,1))        # reverse steering rate (angular)
    
        equations = vertcat( v * np.sin(psi), 
                           v * np.cos(psi),
                           a_rev ,
                           (v/self.Lb_rev) * np.tan(delta),
                           w_rev
                         )
    ...
    def control_callback(self, model):
        ...
        def mpc_controller(self, model):
            print('Here i just entered the controller setup')
            mpc = do_mpc.controller.MPC(self.model)
            mpc_rev = do_mpc.controller.MPC(self.model_rev)
            ...
            # Objective function:
            self.cost = (xc - x_set) ** 2 + (yc - y_set)**2 + (v - v_set)**2
            
            print("Cost ::", self.cost)

            mterm = self.cost                                                  # terminal cost
            lterm = self.cost                                                  # stage cost
            ...
            mpc.bounds['lower','_u','a_rev'] = -5
            mpc.bounds['lower','_u','w'] = -10
            mpc.bounds['lower','_u','w_rev'] = -10
            mpc.bounds['upper','_u','a'] = 10
            mpc.bounds['upper','_u','a_rev'] = 5
            mpc.bounds['upper','_u','w'] = 10
            mpc.bounds['upper','_u','w_rev'] = 10
