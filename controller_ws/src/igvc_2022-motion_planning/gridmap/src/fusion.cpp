...
const float FinMapRes  = 0.3;                              //map resolution(m/cell)
...
int LaneMapW, LaneMapH;  
float LaneMapRes;                                  //lane map size


std_msgs::Int8MultiArray LaneMap, GridMapArr;
ros::Publisher pub_local_map, pub_cost_map;
...
{	//cout << "LIDAR CALLBACK WORKING\t";
	...
	if(true){
		...
		//cout<< "in lidarcallback: numrays" << NumLidarRays  << "lidarxy ka size:" << LidarXY.size() << "\n";
	  ...
	  GCostMap.header.stamp = ros::Time::now();
		...
		// LANE ADDING
		 ...
		 for(int i=0;i<LaneMapH;i++)//adding lane data
		 {
		 	...
		 	{	
				int Yco = j*LaneMapRes/MapRes + LaneMapShift;
				int Xco = i*LaneMapRes/MapRes + CamDist;
				...
				if(LaneMap.data[(i)*LaneMapW+j]==127)
				{
		 			GridMap.data[(Xco+CARCEN_X)*MapWidth+(Yco+CARCEN_Y)] = LaneCost;
		 		}
		 	}
		 }
   ...
   Mat GridMapImg = Mat::zeros(MapHeight,MapWidth,CV_8UC1);
   for(int a=0;a<MapHeight;a++)
     {
      for(int b=0;b<MapWidth;b++)
      {
        //int i_= MapHeight - a;
        GridMapImg.at<uchar>(a,b) = GridMap.data[a*MapWidth+b]*(-2.55)+255;
      }
    }
    ...
    for(int a=0;a<MapHeight;a++)
     {
      ...
      {
        //int i_= MapHeight - a;
       //GCostMap.data[a*MapWidth+b] =   GridMapImg.at<uchar>(a,b);
      	GCostMap.data[a*MapWidth+b] =   0;

      }
    }
   //imshow("Display window", GridMapImg);
    //int k = waitKey(0);
    //imwrite("/home/student/students/aadya/catkin_ws/src/path_planner-master/maps/image1.png", GridMapImg);
		pub_local_map.publish(GridMap);
		pub_cost_map.publish(GCostMap);
...
{	cout << "Start with this node\n";
	ros::init(argc,argv,"local_map_fused");

	ros::NodeHandle LRSubNode, LidarSubNode, MapPubNode, CostMapPubNode;
	...
	pub_cost_map = CostMapPubNode.advertise<nav_msgs::OccupancyGrid>("/scan/cost_map",1);  //Do not comment this, otherwise code fails with controls package


	ros::Rate RosLoopRate(14);
	while(ros::ok())
	{
